...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    print('plotting results over {} / {} sequences'.format(valid_sequence.long().sum().item(), valid_sequence.shape[0]))

    print('generating plots for {}'.format(report_name))
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             'ylabel': 'Overlap Precision [%]', 'xlim': (0, 1.0), 'ylim': (0, 85), 'title': 'Success'}
...
def plot_got_success(trackers, report_name):
    ...
    for trk_id, trk in enumerate(trackers):
        ...
        if os.path.isfile(json_path):
            ...
            raise Exception('ERROR: report not found at {}'.format(json_path))
        ...
        if 'succ_curve' in eval_data.keys():
            ...
            raise Exception('ERROR: invalid JSON file {}'.format(json_path))
