...
class TensorDict(OrderedDict):
    ...
    def __getattr__(self, name):
        if not hasattr(torch.Tensor, name):
            raise AttributeError('ERROR: \'TensorDict\' object has not attribute \'{}\''.format(name))
...
class TensorList(list):
    ...
    def __getattr__(self, name):
        if not hasattr(torch.Tensor, name):
            raise AttributeError('ERROR: \'TensorList\' object has not attribute \'{}\''.format(name))
...
def tensor_operation(op):
    ...
    def oplist(*args, **kwargs):
        if len(args) == 0:
            raise ValueError('ERROR: must be at least one argument without keyword (i.e. operand)')
